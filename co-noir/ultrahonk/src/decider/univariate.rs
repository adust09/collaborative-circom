...
impl<F: PrimeField, const SIZE: usize> Univariate<F, SIZE> {
    ...
    pub(crate) fn evaluate(&self, u: F) -> F {
        ...
        let lagrange_denominators =
            Barycentric::construct_lagrange_denominators::<F>(SIZE, &self.evaluations);

        let denominator_inverses = Barycentric::construct_denominator_inverses::<F>(
            SIZE,
            &self.evaluations,
            &lagrange_denominators,
        );
        ...
        // Compute each term v_j / (d_j*(x-x_j)) of the sum
        for (i, &inverse) in denominator_inverses.iter().enumerate().take(SIZE) {
